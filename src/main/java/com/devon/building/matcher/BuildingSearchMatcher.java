...
public class BuildingSearchMatcher {
  ...
  public boolean matches(Building building, BuildingSearchRequest request) {
    ...
    if (request.getRentPriceFrom() != null
        && (long) building.getPrice() < request.getRentPriceFrom()) {
      return false;
    }
  ...
  private boolean containsIgnoreCase(String buildingValue, String searchValue) {
    ...
    return buildingValue != null && buildingValue.toLowerCase().contains(searchValue.toLowerCase());
  }
  ...
  private boolean matchesRentArea(Building building, BuildingSearchRequest request) {
    ...
    return building.getRentAreas().stream()
        .anyMatch(
            rentArea ->
                (request.getRentAreaFrom() == null
                        || rentArea.getValue() >= request.getRentAreaFrom())
                    && (request.getRentAreaTo() == null
                        || rentArea.getValue() <= request.getRentAreaTo()));
  }
}
