...
public class BuildingRepositoryCustom {
  ...
  public PaginationResult<Building> queryBuilding(
      int page, int maxResult, int maxNavigationPage, BuildingSearchRequest buildingSearchRequest) {
    StringBuilder sql =
        new StringBuilder("select distinct b from ")
            .append(Building.class.getName())
            .append(" b where 1 = 1");
    StringBuilder countSql =
        new StringBuilder("select count(distinct b.id) from ")
            .append(Building.class.getName())
            .append(" b where 1 = 1");
  ...
  private void appendConditions(
    ...
    appendRange(
        ...
        "rentPriceFrom",
        "rentPriceTo",
        request.getRentPriceFrom(),
        request.getRentPriceTo());
    appendContains(
        sql, countSql, parameters, "b.managerName", "managerName", request.getManagerName());
    appendContains(
        sql,
        countSql,
        parameters,
  ...
  private void appendStaff(
      StringBuilder sql, StringBuilder countSql, Map<String, Object> parameters, Long staffId) {
    if (staffId == null) {
      return;
    }
  ...
  private void appendRentArea(
    ...
    StringBuilder clause =
        new StringBuilder(" and exists (select 1 from b.rentAreas rentArea where 1 = 1");
