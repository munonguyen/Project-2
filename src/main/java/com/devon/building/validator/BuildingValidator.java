...
public class BuildingValidator {
    // Em thấy @Valid nó chỉ kiểm tra đơn thuần
    ...
    private void validateRentArea(BuildingDTO buildingDTO, List<String> details) {
        ...
        String trimmed = rentArea.replaceAll("\\s", "");
        if (!trimmed.matches("\\d+(,\\d+)*")) {
            details.add("rentArea: Diện tích thuê không hợp lệ (VD: 100,200,300)");
            return;
        }
        for (String value : trimmed.split(",")) {
            long parsed = Long.parseLong(value);
            if (parsed <= 0) {
                details.add("rentArea: Diện tích thuê phải là số dương");
                return;
            }
        }
    }
